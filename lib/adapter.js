 ...
 */
MongoAdapter.prototype._get = async function(key, callback) {
  try{
  const shard = await this._model.findOne({ hash: key }).exec();

  if (!shard) {
    return callback(new Error('Shard data not found'));
  }
    return callback(null, shard.toObject());
  }catch(err){
    return callback(err);
  }
};
...
MongoAdapter.prototype._put = function(key, item, callback) {
  //  Shard.create is a custom method, it allows callbacks.
